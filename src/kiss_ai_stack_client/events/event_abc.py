...
class EventAbc(ABC):
    ...
    for an AI stack. These operations include authorization, lifecycle
    ...
    async def authorize_stack(self, client_id: Optional[str] = None, client_secret: Optional[str] = None,
        ...
        """
        Authorizes the stack using provided credentials and scope. If you have saved a previous client_id and
        ...
        :param client_id: (Optional[str]): The client ID for the stack.
    ...
    async def destroy_stack(self, data: Optional[str] = None) -> GenericResponseBody:
        """
        Destroys the stack and cleans up any associated resources.
    ...
    async def bootstrap_stack(self, data: Optional[str] = None) -> GenericResponseBody:
        """
        Bootstraps the stack session in server by initializing necessary components and configurations.
